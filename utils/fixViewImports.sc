...
def processScalaFile(path: os.Path): Unit = {
  ...
  subpackages.foreach { subpackage =>
    ...
    val modifiedCode = code.linesIterator.toSeq
      .map { l =>
        var line = l
    ...
    if (importFound && (pagesImports.nonEmpty || generalImports.nonEmpty)) {
      ...
      if (generalImports.nonEmpty) {
        println(s"${AnsiColor.GREEN}Replacing general imports ${AnsiColor.WHITE}${generalImports
            .mkString(", ")}${AnsiColor.GREEN} at ${AnsiColor.MAGENTA}$path${AnsiColor.RESET}")
      }
      if (pagesImports.nonEmpty) {
        println(
          s"${AnsiColor.GREEN}Replacing ${AnsiColor.CYAN}$subpackage${AnsiColor.GREEN} with imports ${AnsiColor.WHITE}${pagesImports
              .mkString(", ")}${AnsiColor.GREEN} at ${AnsiColor.MAGENTA}$path${AnsiColor.RESET}"
        )
      }
      hasChanges = true
