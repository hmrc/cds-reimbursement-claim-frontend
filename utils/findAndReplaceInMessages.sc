...
println(s"""Searching messages in $CYAN$messagesFilePath$RESET using criteria: 
  ${
    if ifKeyContains.nonEmpty then
      ifKeyContains.map(k => s"\'$YELLOW$k$RESET\'").mkString("- key must contain ", " and ", "")
    else ""
  }
  ${
    if ifKeyNotContains.nonEmpty then
      ifKeyNotContains.map(k => s"\'$YELLOW$k$RESET\'").mkString("- key must NOT contain ", " nor ", "")
    else ""
  }
  ${
    if ifValueContains.nonEmpty then
      ifValueContains.map(k => s"\'$YELLOW$k$RESET\'").mkString("- message must contain ", " and ", "")
    else ""
  }
  ${
    if ifValueNotContains.nonEmpty then
      ifValueNotContains.map(k => s"\'$YELLOW$k$RESET\'").mkString("- message must NOT contain ", " nor ", "")
    else ""
  }
...
messages.foreach { case (key, value) =>
  ...
  if ifKeyContains.forall(s => key.contains(s))
  ...
  then
    val newValue = value.replace(
      existingWord,
      if save then replacementWord else s"$RED$CROSSED$existingWord$RESET$GREEN$replacementWord$YELLOW"
    )
