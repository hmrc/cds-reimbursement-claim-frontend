...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.BasisOfOverpaymentClaim.{IncorrectAdditionalInformationCode, IncorrectExciseValue}
...
class ExciseCodesSpec extends AnyWordSpec with Matchers {
  ...
  "ExciseCodes" should {
    "Return all excise codes" in {
      ...
      val acc14 = Functor[Id].map(sample[ImportDeclaration])(dd =>
      ...
      val codes: Set[BasisOfOverpaymentClaim] =
        BasisOfOverpaymentClaim.excludeNorthernIrelandClaims(
          true,
          claimAnswers.importDeclaration
        )
