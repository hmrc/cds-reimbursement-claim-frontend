...
class OverpaymentsScheduledJourneyFormatSpec
    extends AnyWordSpec
    with JsonFormatTest
    with Matchers
  ...
  "OverpaymentsScheduledJourney.Answers" should {
    ...
    "serialize finalized journey into a JSON format" in {
      forAll(completeJourneyGen) { journey =>
        ...
        val deserializedJourney =
          Json.parse(Json.stringify(Json.toJson(finalizedJourney))).as[OverpaymentsScheduledJourney]
