...
class ValidatorSpec extends AnyWordSpec with ScalaCheckPropertyChecks with Matchers {
  ...
  "Validator" should {
    ...
    "and combinator with super/sub types" in {
    ...
    "or combinator with super/sub types" in {
    ...
    "Validator.all combines provided validators to verify if all checks passes" in {
    ...
    "Validator.all combines provided checks to verify if all checks passes" in {
    ...
    "Validator.all(with error prefix) combines provided validators to verify if all checks passes" in {
      val nonEmptyStringValidator    = Validator.checkIsTrue[String](_.nonEmpty, "string must be non-empty")
      val emptyStringValidator       = Validator.checkIsTrue[String](_.isEmpty(), "string must be empty")
    ...
    "Validator.all(with error prefix) combines provided checks to verify if all checks passes" in {
      val nonEmptyStringValidator    = checkIsTrue[String](_.nonEmpty, "string must be non-empty")
      val emptyStringValidator       = checkIsTrue[String](_.isEmpty(), "string must be empty")
      val validate: Validate[String] =
        Validator.allWithPrefix("foo: ", nonEmptyStringValidator, emptyStringValidator)
    ...
    "Validator.all(with calculated error prefix) combines provided validators to verify if all checks passes" in {
      val nonEmptyStringValidator = Validator.checkIsTrue[String](_.nonEmpty, "string must be non-empty")
      val emptyStringValidator    = Validator.checkIsTrue[String](_.isEmpty(), "string must be empty")
    ...
    "Validator.all(with calculated error prefix) combines provided checks to verify if all checks passes" in {
      val nonEmptyStringValidator = checkIsTrue[String](_.nonEmpty, "string must be non-empty")
      val emptyStringValidator    = checkIsTrue[String](_.isEmpty(), "string must be empty")
    ...
    "Validator.allWithShortCircuit combines provided validators to verify if all checks passes but breaks short if any failure" in {
    ...
    "Validator.any combines provided validators to verify if any of the checks passes" in {
      ...
      forAll(Gen.alphaChar, Gen.numChar) { (a: Char, d: Char) =>
        whenever(a.isLower) {
          expect(
    ...
    "Validator.any(with error prefix) combines provided validators to verify if any of the checks passes" in {
      val hasDigitValidator     = Validator.checkIsTrue[String](_.exists(_.isDigit), "some characters must be digits")
      val hasLowerCaseValidator =
        Validator.checkIsTrue[String](_.exists(_.isLower), "some characters must be lower case")
    ...
    "Validator.any(with calculated error prefix) combines provided validators to verify if any of the checks passes" in {
      val hasDigitValidator     = Validator.checkIsTrue[String](_.exists(_.isDigit), "some characters must be digits")
      val hasLowerCaseValidator =
        Validator.checkIsTrue[String](_.exists(_.isLower), "some characters must be lower case")
    ...
    "Validator.conditionally runs the test and follows with either first or second check" in {
    ...
    "Validator.whenTrue runs the test and if true then follows with the next check" in {
      val validateOnlyDigits          = Validator.checkIsTrue[String](_.forall(_.isDigit), "all characters must be digits")
      def validateLength(length: Int) =
        Validator.checkIsTrue[String](_.length() == length, s"must have $length characters")
      val validate: Validate[String]  =
    ...
    "Validator.whenFalse runs the test and if false then tries the next check" in {
      val validateNonEmpty           = Validator.checkIsTrue[String](_.nonEmpty, "must be non empty string")
      val validateAllUpperCase       = Validator.checkIsTrue[String](_.forall(_.isUpper), "all characters must be upper case")
      val validate: Validate[String] =
    ...
    "Validator.when runs the guard check and follows with either first or second check" in {
    ...
    "Validator.whenValid runs the guard check and if valid then follows with the next check" in {
      val validateStartsWithZero      =
        Validator.checkIsTrue[String](_.headOption.contains('0'), "first character must be a Zero")
      val validateOnlyDigits          = Validator.checkIsTrue[String](_.forall(_.isDigit), "all characters must be digits")
      def validateLength(length: Int) =
    ...
    "Validator.whenInvalid runs the guard check and if invalid then tries the next check" in {
      val validateStartsWithZero      =
        Validator.checkIsTrue[String](_.headOption.contains('0'), "first character must be a Zero")
      val validateNonEmpty            = Validator.checkIsTrue[String](_.nonEmpty, "must be non empty string")
      val validateAllUpperCase        = Validator.checkIsTrue[String](_.forall(_.isUpper), "all characters must be upper case")
    ...
    "Validator.product combines provided validators to verify tuples of values" in {
    ...
    "Validator.checkIsTrue returns Valid only if condition fulfilled" in {
      val validate =
    ...
    "Validator.checkIsFalse returns Valid only if condition not met" in {
      val validate =
    ...
    "Validator.checkEquals returns Valid only if values are the same" in {
      val validate =
    ...
    "Validator.checkNotEquals returns Valid only if values are not the same" in {
      val validate =
        Validator
          .checkNotEquals[Foo, Int](_.bar.toInt, _.bazOpt.getOrElse(0), "foo.bar must be not the same as foo.baz")
    ...
    "Validator.checkIsDefined returns Valid only if condition returns Some" in {
      ...
      forAll { (int: Int) =>
        expect(validate(Some(int)).isValid)
      }
    }

    "Validator.checkIsEmpty returns Valid only if condition returns None" in {
      val validate: Validate[Option[Int]] =
      ...
      forAll { (int: Int) =>
        expect(validate(Some(int)).isInvalid)
      }
    }

    "Validator.checkFromEither returns Valid only if condition returns Right" in {
    ...
    "Validator.checkFromEither(with error prefix) returns Valid only if condition returns Right" in {
      val validate: Validate[Int] =
        Validator
    ...
    "Validator.checkWith returns Valid only if extracted property passes check" in {
    ...
    "Validator.checkWithImplicitly returns Valid only if extracted property passes check" in {
    ...
    "Validator.checkUsing(with error prefix) returns Valid only if nested validator returns Valid" in {
      val nonEmptyStringValidator = Validator.checkIsTrue[String](_.nonEmpty, "string must be non-empty")
      val validate: Validate[Foo] =
        Validator
    ...
    "Validator.checkEither returns Valid only if nested validator returns Valid" in {
    ...
    "Validator.checkIfSome returns Valid only if nested validator returns Valid" in {
    ...
    "Validator.checkIfSome(with invalid if None) returns Valid only if nested validator returns Valid" in {
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Foo]  =
    ...
    "Validator.checkIfSome(with error prefix) returns Valid only if nested validator returns Valid" in {
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Foo]  =
        Validator
    ...
    "Validator.checkIfSome(with error prefix and invalid if none) returns Valid only if nested validator returns Valid" in {
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Foo]  =
        Validator
    ...
    "Validator.checkEach returns Valid only if all elements of the sequence passes check" in {
    ...
    "Validator.checkEach(with error prefix fx) returns Valid only if all elements of the sequence passes check" in {
      case class Ints(seq: Seq[Int])
      val negativeIntegerValidator = Validator.checkIsTrue[Int](_ < 0, "must be negative integer")
      val validate: Validate[Ints] =
        Validator
      ...
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(0, Integer.MAX_VALUE))) { (ints: Seq[Int]) =>
        ...
        expect(result.isInvalid, result.errorsCount == ints.size)
      }
    }

    "Validator.checkEach(with error prefix fx) returns Valid only if all elements of the sequence passes check (2)" in {
      case class Ints(seq: Seq[Int])
      val negativeIntegerValidator = Validator.checkIsTrue[Int](_ < 0, "must be negative integer")
      val validate: Validate[Ints] =
      ...
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(Integer.MIN_VALUE, -1))) { (ints: Seq[Int]) =>
        expect(validate(Ints(ints)).isValid)
      }
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(0, Integer.MAX_VALUE))) { (ints: Seq[Int]) =>
        val result = validate(Ints(ints))
        expect(
    ...
    "Validator.checkEachIfNonEmpty returns Valid only if all elements of the sequence passes check" in {
      case class Ints(seq: Seq[Int])
      val negativeIntegerValidator = Validator.checkIsTrue[Int](_ < 0, "must be negative integer")
      val validate: Validate[Ints] =
      ...
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(0, Integer.MAX_VALUE))) { (ints: Seq[Int]) =>
        expect(validate(Ints(ints)).errorsSummaryOption == Some("must be negative integer"))
      }
    }

    "Validator.checkEachIfNonEmpty(with error prefix fx) returns Valid only if all elements of the sequence passes check" in {
      case class Ints(seq: Seq[Int])
      val negativeIntegerValidator = Validator.checkIsTrue[Int](_ < 0, "must be negative integer")
      val validate: Validate[Ints] =
        Validator
    ...
    "Validator.checkEachIfNonEmpty(with error prefix fx) returns Valid only if all elements of the sequence passes check (2)" in {
      case class Ints(seq: Seq[Int])
      val negativeIntegerValidator = Validator.checkIsTrue[Int](_ < 0, "must be negative integer")
      val validate: Validate[Ints] =
    ...
    "Validator.checkEachIfSome returns Valid only if None or all elements of the sequence passes check" in {
      ...
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(Integer.MIN_VALUE, 0))) { (ints: Seq[Int]) =>
        expect(validate(Ints(Some(ints))).errorsSummaryOption == Some("must be positive integer"))
      }
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(1, Integer.MAX_VALUE))) { (ints: Seq[Int]) =>
        expect(validate(Ints(Some(ints))).isValid)
      }
    }

    "Validator.checkEachIfSome returns Valid only if sequence is defined and all elements of the sequence passes check" in {
      case class Ints(seqOpt: Option[Seq[Int]])
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Ints] =
    ...
    "Validator.checkEachIfSome(with error prefix) returns Valid only if sequence is None or all elements of the sequence passes check" in {
      case class Ints(seqOpt: Option[Seq[Int]])
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Ints] =
      ...
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(Integer.MIN_VALUE, 0))) { (ints: Seq[Int]) =>
        expect(validate(Ints(Some(ints))).errorsCount == ints.size)
      }
      forAll(Gen.nonEmptyContainerOf[Seq, Int](Gen.chooseNum(1, Integer.MAX_VALUE))) { (ints: Seq[Int]) =>
        expect(validate(Ints(Some(ints))).isValid)
      }
    ...
    "Validator.checkEachIfSome(with error prefix) returns Valid only if sequence is defined and all elements of the sequence passes check" in {
      case class Ints(seqOpt: Option[Seq[Int]])
      val positiveIntegerValidator = Validator.checkIsTrue[Int](_ > 0, "must be positive integer")
      val validate: Validate[Ints] =
        Validator.checkEachIfSomeWithErrorPrefix[Ints, Int](
    ...
    "Validator.checkIfAllDefined returns Valid only if all of the provided functions returns Some" in {
    ...
    "Validator.checkIfAllEmpty returns Valid only if all of the provided functions returns None" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfAllOrNoneDefined returns Valid only if all of the provided functions returns None" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfAtLeastOneIsDefined returns Valid only if at least one of the provided functions returns Some" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfAtMostOneIsDefined returns Valid only if at most one of the provided functions returns Some" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfOnlyOneIsDefined returns Valid if only one of the provided functions returns Some" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfOnlyOneSetIsDefined returns Valid if only one of the provided set of functions have all results defined" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfOnlyOneSetIsDefined returns Valid if only one of the provided set of functions have all results defined (2)" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfAllTrue returns Valid only if all of the provided functions return true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
    ...
    "Validator.checkIfAllFalse returns Valid only if all of the provided functions return true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
        .checkIfAllFalse[Bar](
          Seq(_.a.isDefined, _.b.exists(_ > 0), _.c.contains(false), _.d.exists(_.size > 1)),
    ...
    "Validator.checkIfAtLeastOneIsTrue returns Valid if only if at least one of the provided functions return true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
    ...
    "Validator.checkIfAtMostOneIsTrue returns Valid only if at most one of the provided functions return true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
    ...
    "Validator.checkIfOnlyOneIsTrue returns Valid if only one of the provided functions return true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
    ...
    "Validator.checkIfOnlyOneSetIsTrue returns Valid if only one of the provided set of functions is all true" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
    ...
    "Validator.checkIfOnlyOneSetIsTrue returns Valid if only one of the provided set of functions is all true (2)" in {
      case class Bar(a: Option[String], b: Option[Int], c: Option[Boolean], d: Option[Seq[Int]])
      val validate = Validator
        .checkIfOnlyOneSetIsTrue[Bar](
          Seq(
    ...
    "example" in {
      ...
      val addressCheck = Validator.all[Address](
      ...
      val phoneNumberCheck = Validator.all[PhoneNumber](
        checkWith(_.prefix, phoneNumberPrefixCheck),
        checkWith(_.number, phoneNumberValueCheck)
      )

      val contactCheck = Validator.all[Contact](
