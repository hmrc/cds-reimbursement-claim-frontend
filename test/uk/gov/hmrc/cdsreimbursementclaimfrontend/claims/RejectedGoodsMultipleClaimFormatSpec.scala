...
class RejectedGoodsMultipleClaimFormatSpec
    extends AnyWordSpec
    with JsonFormatTest
    with Matchers
  ...
  "RejectedGoodsMultipleClaim.Answers" should {
    ...
    "serialize finalized claim into a JSON format" in {
      forAll(completeClaimGen) { claim =>
        ...
        val deserializedClaim =
          Json.parse(Json.stringify(Json.toJson(finalizedClaim))).as[RejectedGoodsMultipleClaim]
