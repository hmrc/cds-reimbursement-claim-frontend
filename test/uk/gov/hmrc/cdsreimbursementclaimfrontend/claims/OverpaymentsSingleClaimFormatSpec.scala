...
class OverpaymentsSingleClaimFormatSpec
    extends AnyWordSpec
    with JsonFormatTest
    with Matchers
  ...
  "OverpaymentsSingleClaim.Answers" should {
    ...
    "serialize finalized claim into a JSON format" in {
      forAll(completeClaimGen) { claim =>
        ...
        val deserializedClaim =
          Json.parse(Json.stringify(Json.toJson(finalizedClaim))).as[OverpaymentsSingleClaim]
