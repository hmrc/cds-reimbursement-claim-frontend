...
class RejectedGoodsScheduledClaimFormatSpec
    extends AnyWordSpec
    with JsonFormatTest
    with Matchers
  ...
  "RejectedGoodsScheduledClaim.Answers" should {
    ...
    "serialize finalized claim into a JSON format" in {
      forAll(completeClaimGen) { claim =>
        ...
        val deserializedClaim =
          Json.parse(Json.stringify(Json.toJson(finalizedClaim))).as[RejectedGoodsScheduledClaim]
