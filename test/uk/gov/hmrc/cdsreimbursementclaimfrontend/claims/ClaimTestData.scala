  ...
  implicit class EitherOps[A](val either: Either[String, A]) {
    def getOrFail(implicit pos: org.scalactic.source.Position): A =
      either.fold(
        error =>
          throw new Exception(
            s"Claim construction has failed because of $error at ${pos.fileName}:${pos.lineNumber}"
          ),
        identity
      )
      ...
      either.fold(
        error =>
          if error === expectedError then ()
            ...
            throw new Exception(
              s"Claim construction in ${pos.fileName}:${pos.lineNumber} has failed as expected, but error was different: $error"
            )
        ,
        _ =>
          throw new Exception(
            s"Expected failure but claim construction succeeded in ${pos.fileName}:${pos.lineNumber}"
          )
      )
  }
