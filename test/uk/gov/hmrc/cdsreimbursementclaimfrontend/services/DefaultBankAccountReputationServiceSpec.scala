...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.{AccountName, AccountNumber, BankAccountDetails, BankAccountType, SortCode}
...
class DefaultBankAccountReputationServiceSpec extends AnyWordSpec with Matchers with MockFactory with BeforeAndAfterEach {
  ...
  val testBankAccountDetails = BankAccountDetails(accountName = AccountName("Testname"),
  ...
  val confirmedBankAccountReputation = BankAccountReputation(accountNumberWithSortCodeIsValid = Yes, accountExists = Some(Yes), nameMatches = Some(Yes))
  ...
  "The DefaultBankAccountReputationService" when {
    ...
    "checkBankAccountReputation" should {
      "delegate to getPersonalAccountReputation when account type is Personal" in {
        ...
        val result = await(testService.checkBankAccountReputation(testBankAccountType, testBankAccountDetails, None).value)
        val expectedResult = Right(testBankAccountReputation)
    ...
    "checkBankAccountReputationV2" should {
      ...
      "fallback to personal reputation check when business account check is not confirmed and personal is confirmed" in {
        val unconfirmedBusinessReputation = BankAccountReputation(accountNumberWithSortCodeIsValid = ReputationResponse.No)
      ...
      "return business reputation when both business and personal checks are not confirmed" in {
        val unconfirmedBusinessReputation = BankAccountReputation(accountNumberWithSortCodeIsValid = ReputationResponse.No)
        val unconfirmedPersonalReputation = BankAccountReputation(accountNumberWithSortCodeIsValid = ReputationResponse.No)
