...
class EoriDetailsConnectorGetCurrentUserEoriDetailsSpec
    extends AnyWordSpec
    with Matchers
    with MockFactory
    with HttpV2Support
  ...
  "EoriDetailsConnector" must {
    ...
    "return details when 200 with eoriXI" in {
      ...
      await(connector.getCurrentUserEoriDetails) shouldBe Some(
    ...
    "return empty when 204" in {
      ...
      await(connector.getCurrentUserEoriDetails) shouldBe None
    }
    ...
    "throw exception when empty response" in {
      ...
      a[EoriDetailsConnector.Exception] shouldBe thrownBy {
        await(connector.getCurrentUserEoriDetails)
      }
    }
    ...
    "throw exception when 4xx response status" in {
      ...
      Try(await(connector.getCurrentUserEoriDetails)) shouldBe Failure(
        new EoriDetailsConnector.Exception(
          "Request to GET http://host3:123/foo-claim/eori failed because of 404 case not found"
        )
      )
    }
