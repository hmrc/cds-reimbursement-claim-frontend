...
class RejectedGoodsMultipleClaimConnectorSpec
  ...
  "RejectedGoodsMultipleClaimConnector" must {
    ...
    "return caseNumber when successful call" in {
      ...
      await(connector.submitClaim(sampleRequest, false)) shouldBe RejectedGoodsMultipleClaimConnector.Response("ABC123")
    }
    ...
    "retry claim submission with a free text input extracted as a separate files when 403 FORBIDDEN and none special circumstances" in {
      ...
      await(connector.submitClaim(request, true)) shouldBe RejectedGoodsMultipleClaimConnector.Response(
        "ABC123"
      )
    }

    import RejectedGoodsMultipleClaimConnector.*
