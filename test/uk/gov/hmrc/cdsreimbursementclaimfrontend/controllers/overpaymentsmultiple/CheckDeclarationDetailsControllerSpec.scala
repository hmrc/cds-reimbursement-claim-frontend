...
class CheckDeclarationDetailsControllerSpec
  ...
  val session =
    SessionData.empty.copy(overpaymentsMultipleClaim = Some(OverpaymentsMultipleClaim.empty(exampleEori)))
    ...
    "Submit Check Declaration Details page" must {
      ...
      "submit" in {
        inSequence {
          ...
          mockStoreSession(
            session.copy(
              overpaymentsMultipleClaim = session.overpaymentsMultipleClaim.map(_.withEnterContactDetailsMode(true))
            )
          )(Right(()))
        }
        ...
        val claim = initialClaim
          .submitMovementReferenceNumberAndDeclaration(
            ...
            buildImportDeclaration()
              ...
              .withDeclarantEori(initialClaim.getLeadImportDeclaration.get.getDeclarantEori)
          )
          .getOrFail
