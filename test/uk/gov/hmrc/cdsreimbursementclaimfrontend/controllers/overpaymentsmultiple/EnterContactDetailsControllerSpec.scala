...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.claims.OverpaymentsMultipleClaimGenerators.{buildCompleteClaimGen, emptyClaim}
import uk.gov.hmrc.cdsreimbursementclaimfrontend.controllers.{AuthSupport, PropertyBasedControllerSpec, SessionSupport}
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.{MrnContactDetails, SessionData}
...
class EnterContactDetailsControllerSpec
  ...
  val session: SessionData = SessionData.empty.copy(overpaymentsMultipleClaim = Some(emptyClaim))

  val messageKey: String = "enter-movement-reference-number"

  private def mockCompleteClaim(
    ...
    "Submit Contact Details" must {
      ...
      "submit a valid contact details" in forAll(
        ...
        inSequence {
          ...
          mockGetSession(SessionData(claim))
          mockStoreSession(
            session.copy(overpaymentsMultipleClaim =
              Some(
