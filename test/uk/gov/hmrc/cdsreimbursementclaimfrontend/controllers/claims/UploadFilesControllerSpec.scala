...
class UploadFilesControllerSpec extends PropertyBasedControllerSpec with AuthSupport with SessionSupport {
  ...
  "UploadFilesController" when {
    ...
    for (journey <- JourneyBindable.all)
      s"Show choose files page in $journey" must {
    ...
    for (journey <- JourneyBindable.all)
      s"Redirect to the file upload summary page in $journey" must {

        def performAction(): Future[Result] = controller.summary(journey)(FakeRequest())
    ...
    for (journey <- JourneyBindable.all)
      s"'Choose Files' page submitted the callback in $journey" must {

        def performAction(callback: UploadDocumentsCallback): Future[Result] =
          controller.callback(journey)(FakeRequest().withJsonBody(Json.toJson(callback)))
        ...
        "return 400 if callback rejected because of invalid request" in {
          ...
          val result       = controller.callback(journey)(FakeRequest().withJsonBody(Json.parse("""{"foo":"bar"}""")))
          status(result) shouldBe 400
        }
