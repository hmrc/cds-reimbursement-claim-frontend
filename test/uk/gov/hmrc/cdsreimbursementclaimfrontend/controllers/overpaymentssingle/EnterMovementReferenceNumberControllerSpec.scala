...
class EnterMovementReferenceNumberControllerSpec
  ...
  "Movement Reference Number Controller" when {
    ...
    "Enter MRN page" must {
      ...
      "display the title and the previously saved MRN" in forAll { (mrnAnswer: MRN) =>
        val typeOfClaim = TypeOfClaimAnswer.Individual
        val router      = JourneyExtractor.getRoutes(typeOfClaim, JourneyBindable.Single)
    ...
    "We enter an MRN for the first time or update it with the back button (enterMrnSubmit)" must {
      ...
      "reject an invalid MRN" in {
        ...
        val (session, _) = sessionWithMRNAndTypeOfClaimOnly(None, Some(TypeOfClaimAnswer.Individual))
      ...
      "start a new claim with an MRN, Eori is importer's Eori" in forAll {
        (
          ...
          val (session, foc) =
            sessionWithMRNAndTypeOfClaimOnly(None, Some(TypeOfClaimAnswer.Individual))
          ...
          checkIsRedirect(
            result,
            claimsRoutes.CheckDeclarationDetailsController.show(JourneyBindable.Single)
          )
      }
      ...
      "Update an MRN, Eori is importer's Eori" in forAll {
        (
          ...
          val (session, foc) =
            sessionWithMRNAndTypeOfClaimOnly(mrnAnswer, Some(TypeOfClaimAnswer.Individual))
      ...
      "On submitting the same MRN as before, don't save it, Eori is importer's Eori" in
        forAll(
          ...
          val mrnAnswer      = mrn.some
          val (session, foc) = sessionWithMRNAndTypeOfClaimOnly(mrnAnswer, Some(TypeOfClaimAnswer.Individual))
  ...
  "We update an MRN coming from the Check Your Answer page (changeMrnSubmit)" must {
    ...
    "return to CYA page if the same MRN is submitted" in forAll { (mrn: MRN) =>
      val (session, _) =
        sessionWithCompleteC285Claim(mrn, TypeOfClaimAnswer.Individual)
    ...
    "start a new claim if a different MRN is submitted" in {
      (
        ...
        redirectLocation(
          ...
          .enterImporterEoriNumber(JourneyBindable.Single)
          .url
    }
  }
