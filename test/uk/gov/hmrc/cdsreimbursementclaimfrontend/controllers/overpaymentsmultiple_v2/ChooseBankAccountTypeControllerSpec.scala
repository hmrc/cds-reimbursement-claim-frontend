...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.controllers.overpaymentsmultiple_v1.ChooseBankAccountTypeController
        ...
        (bankAccountType: BankAccountType, declaration: DisplayDeclaration, ndrc: NdrcDetails) =>
          ...
          val sessionCopy = session.copy(overpaymentsMultipleJourney =
            OverpaymentsMultipleJourney
              ...
              .flatMap(
                _.selectAndReplaceTaxCodeSetForReimbursement(updatedDeclaration.getMRN, Seq(TaxCode(ndrc.taxType)))
              )
              .toOption
          )
          ...
          inSequence {
            mockAuthWithNoRetrievals()
            mockGetSession(sessionCopy)
            mockStoreSession(Right(()))
          }

          checkIsRedirect(
            submitBankAccountType(formKey -> bankAccountType.toString),
            routes.EnterBankAccountDetailsController.submit
          )
      }
    }
