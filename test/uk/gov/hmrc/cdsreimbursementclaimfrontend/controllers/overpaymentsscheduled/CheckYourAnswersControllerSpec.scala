...
class CheckYourAnswersControllerSpec
  ...
  def validateConfirmationPage(doc: Document, claim: OverpaymentsScheduledClaim, caseNumber: String): Assertion = {
  ...
  "Check Your Answers Controller" when {
    ...
    "Show check your answers page" must {
      ...
      "display the page if claim has complete answers" in {
        forAll(completeClaimGen) { claim =>
          ...
          val updatedSession = SessionData.empty.copy(overpaymentsScheduledClaim = Some(claim))
          inSequence {
      ...
      "redirect to the submission confirmation page if claim already finalized" in {
        forAll(completeClaimGen) { claim =>
          val updatedSession =
            SessionData.empty.copy(overpaymentsScheduledClaim =
      ...
      "redirect to the confirmation page if success" in {
        forAll(completeClaimGen) { claim =>
          ...
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(updatedSession)
            mockSubmitClaim(OverpaymentsScheduledClaimConnector.Request(output))(
              Future.successful(OverpaymentsScheduledClaimConnector.Response("foo-123-abc"))
            )
            mockWipeOutCall()
            mockStoreSession(
              updatedSession.copy(overpaymentsScheduledClaim =
    ...
    "Show confirmation page" must {
      ...
      "display the page if claim has been finalized" in {
        forAll(completeClaimGen, genCaseNumber) { (claim, caseNumber) =>
          val updatedSession =
            SessionData.empty.copy(overpaymentsScheduledClaim = claim.finalizeClaimWith(caseNumber).toOption)
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(updatedSession)
          }
