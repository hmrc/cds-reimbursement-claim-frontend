...
package uk.gov.hmrc.cdsreimbursementclaimfrontend.controllers.overpaymentssingle_v1
    ...
    "successfully submit bank account type" when {
      "one of the options selected" in forAll { bankAccountType: BankAccountType =>
        inSequence {
          ...
          mockStoreSession(
            session.copy(
              overpaymentsSingleJourney = journeyWithMrnAndDeclaration.submitBankAccountType(bankAccountType).toOption
            )
          )(Right(()))
        }
