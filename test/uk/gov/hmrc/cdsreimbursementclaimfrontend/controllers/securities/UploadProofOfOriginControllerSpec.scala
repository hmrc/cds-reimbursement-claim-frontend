    ...
    "Show page" must {
      ...
      "redirect to 'Add proof of origin' when some file uploaded already" in forSomeWith(
        ...
        inSequence {
          mockAuthWithDefaultRetrievals()
          mockGetSession(
            SessionData(claim.receiveProofOfOriginDocuments(claim.answers.nonce, exampleUploadedFiles).getOrFail)
          )
      ...
      "redirect to 'Add proof of origin' if claim has complete answers" in {
        forAll(completeClaimOnlyNidacGen) { claim =>
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(SessionData(claim))
            mockInitializeCall(claim.answers.proofOfOriginDocuments)
          }
      ...
      "return 204 if callback accepted" in forSomeWith(
        ...
        inSequence {
          ...
          mockStoreSession(
            SessionData(
              claim
                .receiveProofOfOriginDocuments(claim.answers.nonce, exampleUploadedFiles)
                .getOrFail
            )
          )(Right(()))
        }
