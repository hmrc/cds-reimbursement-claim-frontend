...
class UploadProofOfOriginControllerSpec
  ...
  val expectedUploadDocumentsLocation: String = "http://foo:123/bar/upload-proof-of-origin"
  ...
  val controller: UploadProofOfOriginController = instanceOf[UploadProofOfOriginController]
  ...
  "UploadProofOfOriginController" when {
    ...
    "Show page" must {
      ...
      "redirect to 'Add proof of origin' when some file uploaded already" in forSomeWith(
        ...
        inSequence {
          mockAuthWithDefaultRetrievals()
          mockGetSession(
            SessionData(journey.receiveProofOfOriginDocuments(journey.answers.nonce, exampleUploadedFiles).getOrFail)
          )
      ...
      "redirect to 'Add proof of origin' if journey has complete answers" in {
        forAll(completeJourneyOnlyNidacGen) { journey =>
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(SessionData(journey))
            mockInitializeCall(journey.answers.proofOfOriginDocuments)
          }
    ...
    "'Add proof of origin' submitted the callback" must {
      ...
      "return 204 if callback accepted" in forSomeWith(
        ...
        inSequence {
          ...
          mockStoreSession(
            SessionData(
              journey
                .receiveProofOfOriginDocuments(journey.answers.nonce, exampleUploadedFiles)
                .getOrFail
            )
          )(Right(()))
        }
