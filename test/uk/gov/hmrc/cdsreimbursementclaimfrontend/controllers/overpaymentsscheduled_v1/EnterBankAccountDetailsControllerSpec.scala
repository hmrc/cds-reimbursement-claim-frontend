...
package uk.gov.hmrc.cdsreimbursementclaimfrontend.controllers.overpaymentsscheduled_v1
  ...
  "Enter Bank Account Details Controller" must {
    ...
    "display the page" when {
      ...
      "the user has answered this question before" in forAll(completeJourneyGen) { journey =>
        val updatedSession = session.copy(overpaymentsScheduledJourney = Some(journey))
    ...
    "handle submit requests" when {
      ...
      "the user enters details for the first time" in forAll(genBankAccountDetails) { bankDetails =>
        ...
        val requiredSession = session.copy(overpaymentsScheduledJourney = Some(initialJourney))

        val updatedJourney             = initialJourney.submitBankAccountDetails(bankDetails)
        val updatedSession             = session.copy(overpaymentsScheduledJourney = updatedJourney.toOption)
