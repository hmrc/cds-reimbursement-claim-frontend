...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.{BigDecimalOps, SessionData, TaxCode, TaxCodes}
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.declaration.{DisplayResponseDetail, NdrcDetails}
  ...
  "Enter Claim Controller" when {
    ...
    "Show enter claim page by tax code" must {
      ...
      "redirect to select duties when tax code is not in ndrc details" in {
        ...
        val updatedClaim = RejectedGoodsSingleClaim
          .unsafeModifyAnswers(
            claim,
    ...
    "Submit Enter Claim page" must {
      ...
      "redirect to select duties page when tax code has not been selected" in {
        ...
        val updatedClaim = RejectedGoodsSingleClaim.unsafeModifyAnswers(
          claim,
          _.copy(importDeclaration = Some(importDeclaration), correctedAmounts = Some(correctedAmounts))
        )
