...
class CheckBankDetailsControllerSpec
  ...
  "Check Bank Details Controller" when {
    "Check Bank Details page" must {
      ...
      "display the page" in {
        val claim          = completeClaimCMAEligibleGen.sample.getOrElse(fail("Claim building has failed."))
    ...
    "Submit Check Bank Details page" must {
      ...
      "submit when user selects No" in {
        ...
        inSequence {
          ...
          mockStoreSession(
            sessionToAmend.copy(
              rejectedGoodsSingleClaim = sessionToAmend.rejectedGoodsSingleClaim.map(_.removeBankAccountDetails())
            )
          )(Right(()))
        }
