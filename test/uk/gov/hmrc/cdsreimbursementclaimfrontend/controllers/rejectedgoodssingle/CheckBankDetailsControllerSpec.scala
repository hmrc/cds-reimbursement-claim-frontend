...
class CheckBankDetailsControllerSpec
  ...
  "Check Bank Details Controller" when {
    "Check Bank Details page" must {
      ...
      "display the page" in {
        val journey        = completeJourneyCMAEligibleGen.sample.getOrElse(fail("Journey building has failed."))
    ...
    "Submit Check Bank Details page" must {
      ...
      "submit when user selects No" in {
        ...
        inSequence {
          ...
          mockStoreSession(
            sessionToAmend.copy(
              rejectedGoodsSingleJourney = sessionToAmend.rejectedGoodsSingleJourney.map(_.removeBankAccountDetails())
            )
          )(Right(()))
        }
