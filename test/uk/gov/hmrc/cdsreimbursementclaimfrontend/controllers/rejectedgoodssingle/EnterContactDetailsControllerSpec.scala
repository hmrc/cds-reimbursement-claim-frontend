...
import uk.gov.hmrc.auth.core.{AffinityGroup, AuthConnector, Enrolment}
...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.{Feature, MrnContactDetails, SessionData, contactdetails}
      ...
      "reject an empty contact details form" in forAll(buildCompleteJourneyGen(), genEmail, genName) {
        (journey, email, name) =>
          inSequence {
            ...
            mockGetSession(session.copy(rejectedGoodsSingleJourney = Some(journey.submitContactDetails(None))))
          }
