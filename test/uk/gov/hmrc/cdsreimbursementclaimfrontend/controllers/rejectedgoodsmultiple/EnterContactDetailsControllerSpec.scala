...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.claims.RejectedGoodsMultipleClaimGenerators.{buildCompleteClaimGen, claimWithMrnAndDeclaration}
...
class EnterContactDetailsControllerSpec
  ...
  private def mockCompleteClaim(claim: RejectedGoodsMultipleClaim) =
    inSequence {
    ...
    "Submit Basis for claim page" must {
      ...
      "submit a valid contact details" in forAll(
        ...
        inSequence {
          ...
          mockGetSession(SessionData(claim))
          mockStoreSession(
            session.copy(rejectedGoodsMultipleClaim =
              Some(
