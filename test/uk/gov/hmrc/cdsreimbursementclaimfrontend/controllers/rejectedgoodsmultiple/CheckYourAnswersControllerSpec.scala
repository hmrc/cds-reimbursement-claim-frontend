...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.connectors.{RejectedGoodsMultipleClaimConnector, UploadDocumentsConnector}
...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.views.helpers.{CheckYourAnswersContactDetailsCardSummary, OrdinalNumberMrnHelper}
...
class CheckYourAnswersControllerSpec
  ...
  def validateCheckYourAnswersPage(
    ...
    output: RejectedGoodsMultipleClaim.Output,
    isPrintView: Boolean
  ) = {
  ...
  def validateConfirmationPage(doc: Document, claim: RejectedGoodsMultipleClaim, caseNumber: String): Assertion = {
  ...
  "Check Your Answers Controller" when {
    ...
    "Show check your answers page" must {
      ...
      "display the page if claim has complete answers" in {
        forAll(completeClaimGen) { claim =>
          ...
          val updatedSession = SessionData.empty.copy(rejectedGoodsMultipleClaim = Some(claim))
          inSequence {
      ...
      "redirect to the submission confirmation page if claim already finalized" in {
        forAll(completeClaimGen) { claim =>
          val updatedSession =
            SessionData.empty.copy(rejectedGoodsMultipleClaim =
      ...
      "redirect to the confirmation page if success" in {
        forAll(completeClaimGen) { claim =>
          ...
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(updatedSession)
            mockSubmitClaim(RejectedGoodsMultipleClaimConnector.Request(output))(
              Future.successful(RejectedGoodsMultipleClaimConnector.Response("foo-123-abc"))
            )
            mockWipeOutCall()
            mockStoreSession(
              updatedSession.copy(rejectedGoodsMultipleClaim =
    ...
    "Show confirmation page" must {
      ...
      "display the page if claim has been finalized" in {
        forAll(completeClaimGen, genCaseNumber) { (claim, caseNumber) =>
          val updatedSession =
            SessionData.empty.copy(rejectedGoodsMultipleClaim = claim.finalizeClaimWith(caseNumber).toOption)
          inSequence {
            mockAuthWithDefaultRetrievals()
            mockGetSession(updatedSession)
          }
