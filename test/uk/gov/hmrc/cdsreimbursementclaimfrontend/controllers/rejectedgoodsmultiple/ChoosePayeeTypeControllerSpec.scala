...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.{PayeeType, SessionData}
      ...
      inSequence {
        ...
        mockGetSession(
          maybePayeeType.toList.foldLeft(session)((session, payeeType) =>
            session.copy(rejectedGoodsMultipleClaim = claimWithMrnAndDeclaration.submitPayeeType(payeeType).toOption)
          )
        )
      }
    ...
    "successfully submit bank account type" when {
      "one of the options selected" in forAll { (payeeType: PayeeType) =>
        inSequence {
          ...
          mockStoreSession(
            session.copy(
              rejectedGoodsMultipleClaim = claimWithMrnAndDeclaration.submitPayeeType(payeeType).toOption
            )
          )(Right(()))
        }
