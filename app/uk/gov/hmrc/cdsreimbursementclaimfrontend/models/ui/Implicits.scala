...
import uk.gov.hmrc.cdsreimbursementclaimfrontend.models.ui
...
trait Implicits {
  ...
  implicit class RichFormErrors(formErrors: Seq[FormError])(implicit messages: Messages) {
    ...
    def asErrorLinks(contentConstructor: String => Content): Seq[ErrorLink] =
      formErrors.map { formError =>
        ui.ErrorLink(href = Some(s"#${formError.key}"), content = contentConstructor(errorMessage(formError)))
      }
    ...
    def asErrorMessages(contentConstructor: String => Content): Seq[ErrorMessage] =
      formErrors
        .map { formError =>
          ui.ErrorMessage(content = contentConstructor(errorMessage(formError)))
        }
