...
class MrnSummary extends AnswerSummary[MRN] {
  ...
  def render(answer: MRN)(implicit journey: JourneyBindable, messages: Messages): SummaryList =
    SummaryList(
      Seq(
        SummaryListRow(
          key = Key(Text(messages(s"check-your-answers.mrn.label"))),
            ...
            Actions(
              items = Seq(
                ActionItem(
                  ...
                  visuallyHiddenText = Some(messages(s"check-your-answers.mrn.label"))
                )
              )
            )
          )
