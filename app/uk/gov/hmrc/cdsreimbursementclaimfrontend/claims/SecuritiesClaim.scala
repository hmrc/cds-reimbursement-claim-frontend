...
/** An encapsulated Securities claim logic. The constructor of this class MUST stay PRIVATE to protected integrity of
  ...
  *
  *   - [[SecuritiesClaim.Answers]] - keeps record of user answers and acquired documents
  *   - [[SecuritiesClaim.Output]] - final output of the claim to be sent to backend processing
  */
final class SecuritiesClaim private (
  val answers: SecuritiesClaim.Answers,
  val startTimeSeconds: Long,
  val caseNumber: Option[String] = None,
  val submissionDateTime: Option[LocalDateTime] = None,
  val features: Option[claims.SecuritiesClaim.Features]
) extends ClaimBase
    with CommonClaimProperties
    with DirectFluentSyntax[SecuritiesClaim]
    with SeqUtils
    with ClaimAnalytics {

  type Type = SecuritiesClaim

  val self: SecuritiesClaim = this
  ...
  def selectSecurityDepositId(securityDepositId: String): Either[String, SecuritiesClaim] =
    whileClaimIsAmendableAnd(userCanProceedWithThisClaim) {
  ...
  def removeSecurityDepositId(securityDepositId: String): Either[String, SecuritiesClaim] =
    whileClaimIsAmendableAnd(userCanProceedWithThisClaim) {
      if !isValidSecurityDepositId(securityDepositId) then Left("selectSecurityDepositIds.invalidSecurityDepositId")
      else {
        val updatedClaim = this.copy(
          answers.copy(
            correctedAmounts = answers.correctedAmounts
        ...
        Right(
          this.copy(
            updatedClaim.answers.copy(
              bankAccountDetails =
                if updatedClaim.needsBanksAccountDetailsSubmission then updatedClaim.answers.bankAccountDetails
                else None,
              bankAccountType =
                if updatedClaim.needsBanksAccountDetailsSubmission then updatedClaim.answers.bankAccountType
                else None
            )
          )
        )
  ...
  def clearCorrectedAmounts(securityDepositId: String): Either[String, SecuritiesClaim] =
    whileClaimIsAmendableAnd(userCanProceedWithThisClaim) {
      if !isValidSecurityDepositId(securityDepositId) then Left("submitFullAmountForReclaim.invalidSecurityDepositId")
      else if !isSelectedDepositId(securityDepositId) then
        Left("submitFullAmountForReclaim.securityDepositIdNotSelected")
  ...
  def submitUserXiEori(userXiEori: UserXiEori): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(
  ...
  def submitConsigneeEoriNumber(consigneeEoriNumber: Eori): Either[String, SecuritiesClaim] =
    whileClaimIsAmendable {
  ...
  def submitDeclarantEoriNumber(declarantEoriNumber: Eori): Either[String, SecuritiesClaim] =
    whileClaimIsAmendable {
  ...
  def submitContactDetails(contactDetails: Option[MrnContactDetails]): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(
  ...
  def submitContactAddress(contactAddress: ContactAddress): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(
  ...
  def submitBankAccountDetails(bankAccountDetails: BankAccountDetails): Either[String, SecuritiesClaim] =
    whileClaimIsAmendable {
  ...
  def removeBankAccountDetails(): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(
  ...
  def submitBankAccountType(bankAccountType: BankAccountType): Either[String, SecuritiesClaim] =
    whileClaimIsAmendable {
      if needsBanksAccountDetailsSubmission then
        Right(
          this.copy(
  ...
  def submitDocumentTypeSelection(documentType: UploadDocumentType): Either[String, SecuritiesClaim] =
    whileClaimIsAmendable {
  ...
  def submitCheckDeclarationDetailsChangeMode(enabled: Boolean): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(answers.copy(modes = answers.modes.copy(checkDeclarationDetailsChangeMode = enabled)))
    }

  def submitClaimFullAmountMode(enabled: Boolean): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(answers.copy(modes = answers.modes.copy(claimFullAmountMode = enabled)))
    }

  def resetClaimFullAmountMode(): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(answers.copy(modes = answers.modes.copy(claimFullAmountMode = true)))
    }

  def submitCheckClaimDetailsChangeMode(enabled: Boolean): SecuritiesClaim =
    whileClaimIsAmendable {
  ...
  def submitAdditionalDetailsPageVisited(visited: Boolean): SecuritiesClaim =
    whileClaimIsAmendable {
      this.copy(
        answers.copy(modes =
          answers.modes.copy(
  ...
  def submitCheckYourAnswersChangeMode(enabled: Boolean): SecuritiesClaim =
    whileClaimIsAmendable {
      validate(this)
        .fold(
          _ => this,
  ...
  def finalizeClaimWith(caseNumber: String): Either[String, SecuritiesClaim] =
    whileClaimIsAmendableAnd(userCanProceedWithThisClaim) {
      validate(this)
        .fold(
  ...
  override def equals(obj: Any): Boolean =
    obj match {
      case that: SecuritiesClaim =>
        that.answers === this.answers && that.caseNumber === this.caseNumber
      case _                     => false
    }

  override def hashCode(): Int  = answers.hashCode
  override def toString: String = s"SecuritiesClaim${Json.prettyPrint(Json.toJson(this))}"

  /** Validates the claim and retrieves the output. */

  def toOutput: Either[Seq[String], SecuritiesClaim.Output] =
    validate(this).left
      .map(_.messages)
      .flatMap(_ =>
        (for
...
object SecuritiesClaim extends ClaimCompanion[SecuritiesClaim] {
  ...
  final case class Answers(
    ...
    modes: SecuritiesClaimModes = SecuritiesClaimModes()
  ...
  object Checks extends CommonClaimChecks[SecuritiesClaim] {
    ...
    val reclaimAmountsHasBeenDeclared: Validate[SecuritiesClaim] =
      checkIsTrue[SecuritiesClaim](_.hasCompleteSecuritiesReclaims, INCOMPLETE_SECURITIES_RECLAIMS) &
        checkIsTrue[SecuritiesClaim](
    ...
    val hasMethodOfDisposalIfNeeded: Validate[SecuritiesClaim] =
      conditionally[SecuritiesClaim](
    ...
    val hasExportMRNIfNeeded: Validate[SecuritiesClaim] =
      conditionally[SecuritiesClaim](
    ...
    val hasBillOfDischargeDocumentsIfNeeded: Validate[SecuritiesClaim] =
      conditionally[SecuritiesClaim](
    ...
    val hasProofOfOriginIfNeeded: Validate[SecuritiesClaim] =
      whenTrue[SecuritiesClaim](
    ...
    val additionalDetailsPageVisited: Validate[SecuritiesClaim] =
      checkIsTrue(
        _.answers.modes.additionalDetailsPageVisitedMode,
        ADDITIONAL_DETAILS_NOT_YET_VISITED
      )

    val reasonForSecurityIsIPR: Validate[SecuritiesClaim] =
      checkIsTrue(
        _.reasonForSecurityIsIPR,
        INVALID_REASON_FOR_SECURITY
      )

    val reasonForSecurityIsENU: Validate[SecuritiesClaim] =
      checkIsTrue(
        _.reasonForSecurityIsENU,
        INVALID_REASON_FOR_SECURITY
      )

    val reasonForSecurityIsIPROrENU: Validate[SecuritiesClaim] =
      checkIsTrue(
        j =>
    ...
    val reasonForSecurityIsNidac: Validate[SecuritiesClaim] =
      checkIsTrue(
        _.reasonForSecurityIsNidac,
        INVALID_REASON_FOR_SECURITY
      )

    val needsAddOtherDocuments: Validate[SecuritiesClaim] =
  ...
  implicit val format: Format[SecuritiesClaim] =
    Format(
      ((JsPath \ "answers").read[Answers]
        ...
        and (JsPath \ "features").readNullable[Features])(new SecuritiesClaim(_, _, _, _, _)),
