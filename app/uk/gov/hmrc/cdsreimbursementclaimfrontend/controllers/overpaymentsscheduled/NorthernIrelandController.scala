    ...
    authenticatedActionWithSessionData
      .async { implicit request =>
        withAnswers[YesNo] { (_, answer) =>
          ...
          Ok(northernIrelandAnswerPage(filledForm, Some("scheduled"), postAction))
        }
      }
      ...
      .async { implicit request =>
        withAnswersAndRoutes[YesNo] { (fillingOutClaim, previousAnswer, routes) =>
          ...
          northernIrelandForm
            .bindFromRequest()
            .fold(
              formWithErrors => BadRequest(northernIrelandAnswerPage(formWithErrors, Some("scheduled"), postAction)),
              currentAnswer => {
